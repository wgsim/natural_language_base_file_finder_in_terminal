...
**Tech Stack:** Python 3.12, httpx, rich, prompt-toolkit, keyring, pytest. Conda env: `askfind_env`.
...
```bash
cd <repo-root>
conda run -n askfind_env python -m pip install -e ".[dev]"
conda run -n askfind_env pytest tests/test_cli.py -v
```
...
```bash
conda run -n askfind_env pytest tests/test_config.py -v
```
...
```bash
conda run -n askfind_env pytest tests/test_walker.py -v
```
...
```bash
conda run -n askfind_env pytest tests/test_llm_parser.py -v
```
...
```bash
conda run -n askfind_env pytest tests/test_formatter.py -v
```
...
```bash
conda run -n askfind_env pytest tests/test_cli.py::TestMainIntegration -v
```
...
```bash
conda run -n askfind_env pytest tests/test_pane.py -v
```
...
```bash
conda run -n askfind_env pytest tests/ -v --tb=short
```
...
```bash
# Set up API key
conda run -n askfind_env python -m askfind config set-key

# Test single command mode
conda run -n askfind_env python -m askfind "python files" --root <repo-root>

# Test verbose output
conda run -n askfind_env python -m askfind "python files" -v --root <repo-root>

# Test JSON output
conda run -n askfind_env python -m askfind "python files" --json --root <repo-root>
```
